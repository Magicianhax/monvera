...
async function systemPrompt(ctx: VeraContext): Promise<string> {
  ...
  return [
    ...
    '- Wants an ASSESSMENT of their portfolio ("how does my portfolio look", "review/analyze/check my portfolio", "is my mix healthy") → {"intent":"review_portfolio"} (a deeper engine runs it). No holdings → "reply" honestly. Counter-examples: "open my portfolio", "open portfolio so i can see it", "show me my portfolio" → {"intent":"open","target":"portfolio"} — looking is not reviewing. Explicitly BOTH ("open it and tell me how it\'s doing") → review_portfolio (its card shows the holdings too).',
