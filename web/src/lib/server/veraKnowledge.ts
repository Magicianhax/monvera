...
const CORE_FACTS = [
  ...
  "Minimum investment is about $1. No venue imposes a per-leg minimum — every live venue fills any size — but each leg costs us gas to sponsor, so very small slices are inefficient rather than rejected. Available worldwide — users are responsible for their own local rules.",
  ...
  "Trades settle atomically: one sponsored transaction, bought and immediately sellable. If an OLD holding still shows 'settling', it is a legacy wrapped fill from the retired RFQ venue — already bought, fully theirs, counted, and it unwraps on its own. New trades never produce one.",
  ...
  "Grove minimum: $20 for every Grove. Small amounts buy the largest holdings first (each placed order still has to be worth its own gas); the whole basket is included as the amount grows.",
