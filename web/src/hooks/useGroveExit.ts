...
import { GROVE_MANAGER, buildGroveExitCalls, type GroveExitQuoteJson } from "@/lib/groveManager";

export type GroveExitPhase = "idle" | "quoting" | "signing" | "exiting" | "done" | "error";
...
export function useGroveExit(): UseGroveExit {
  ...
  const getQuote = useCallback(
    async (groveId: string, fractionBps: number) => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(json?.error || json?.message || "Could not price that exit.");
  ...
  const busy = phase === "quoting" || phase === "signing" || phase === "exiting";
  return useMemo(
    () => ({ phase, busy, error, quote, success, getQuote, exit, reset }),
    [phase, busy, error, quote, success, getQuote, exit, reset],
  );
}
