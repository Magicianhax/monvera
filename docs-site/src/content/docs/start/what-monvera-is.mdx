...
| Some USDG | From $1 to start, though a multi-name plan needs about $11 per name. See [Before you invest](/start/before-you-invest/). |

You do not need a gas token. Monvera sponsors the network cost of every transaction, so you never have to hold or top up a second currency to trade. There is no platform fee and no account fee either; Monvera earns a referral fee from the trading venue.
