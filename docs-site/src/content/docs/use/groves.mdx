...
Vera checks every basket every six hours, around the clock — tokenized stocks trade 24/7 — and realigns it **only when it has genuinely drifted** (about five percentage points of weight or more), the move is big enough to be worth what trading it costs, and the on-chain price feeds are fresh. That second test matters most on small baskets: the gas to realign is much the same whatever the size, so on a basket of a few tens of dollars the drift has to roughly double the five-point mark before correcting it leaves you better off than leaving it alone. Larger baskets clear that bar almost as soon as they pass five points. The contract refuses any manager trade against a stale feed, and the real equity feeds gap 52 hours or more over a weekend, so weekend checks often record the drift and wait; that throttle comes from the oracle's actual state, never from a market-hours clock. She also reads the market before acting: when the drift is driven by a name still moving hard, she waits for a calmer window rather than churning your basket through it. Her judgment can only ever veto a rebalance the math already justified; it cannot start, enlarge, or redirect one.
