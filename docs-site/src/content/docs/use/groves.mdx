...
Vera checks every basket every six hours and realigns it **only when it has genuinely drifted** (about five percentage points of weight or more). She acts only during US market hours on weekdays, because the contract refuses stale oracle prices outside them; the real equity price feeds gap 52 hours or more over a weekend, so today that means one acting check per weekday, with the others recording the drift and waiting for a tradable window. She also reads the market before acting: when the drift is driven by a name still moving hard, she waits for a calmer window rather than churning your basket through it. Her judgment can only ever veto a rebalance the math already justified; it cannot start, enlarge, or redirect one.
