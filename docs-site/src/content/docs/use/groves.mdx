...
Auto-manage is designed into the contract but **not switched on yet** — nothing rebalances anyone's basket today, and the Rebalances panel on each Grove's page proves it. When it arrives it will be opt-in per Grove, inside hard caps the contract already enforces (a per-action budget, a lifetime budget, a cooldown between actions, and a per-holding rebalance limit), and revocable instantly at any time — even while the contract is paused.
