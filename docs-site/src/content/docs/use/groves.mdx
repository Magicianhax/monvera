...
Auto-manage is **off by default and opt-in per Grove**. The switch is on the Grove's page in the app: pick a per-action budget and a cadence, see all four caps spelled out, and sign once. From then on Vera checks the basket hourly and realigns it **only when it has genuinely drifted** (about five percentage points of weight or more) — never touching more than the contract lets her on any action: your per-action budget, a lifetime budget, your cooldown, and at most 20% of any single holding per rebalance. Those caps are enforced by the contract itself, not by our servers; the manager key is never trusted. Switching off is instant, needs no cooldown, and works even while the contract is paused.
