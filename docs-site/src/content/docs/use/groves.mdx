...
- **Cost.** A realignment costs roughly the same in gas whatever the basket is worth, so small positions trade less often by arithmetic: on a basket of a few tens of dollars, a one-point adjustment is worth about a dollar and is not worth making. Active management earns its keep from a few hundred dollars up.
