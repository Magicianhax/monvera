...
| 451 | `{"error":"Monvera is not available in your region."}` | A gated API request from a restricted region. |
...
The `451` is enforced in middleware from the Cloudflare `CF-IPCountry` header, before any account exists. Only `/app`, `/api/quote`, `/api/allocate`, `/api/portfolio-review`, `/api/autopilot`, `/api/pimlico`, and `/api/vera` are geo-gated; page paths rewrite to `/restricted` with the same status, and public market, price, and screener reads are not gated at all. The regions and the reason are in [before you invest](/start/before-you-invest/).
