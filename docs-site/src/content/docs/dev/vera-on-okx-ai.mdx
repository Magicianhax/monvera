...
The per-call price is the only fee Vera charges — no execution referral, no spread markup, no percentage of your order. For the record: a 0.5% execution referral fee existed from launch, undisclosed; it was disclosed on 2026-07-20 and removed entirely the same day. If any response ever carries referral swap params, do not execute and check `GET https://vera.monvera.best/` under `pricing.changelog`, where every pricing change is dated. All-in cost examples and the slippage-inclusive worst case are published live there under `costs`.
