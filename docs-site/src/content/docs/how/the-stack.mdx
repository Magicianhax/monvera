...
Tokenized stocks on this chain have no deep pool of their own, so orders route through a spot router that quotes real market makers. Each quote comes back with up to two ways to fill it: one your wallet submits directly, and one the router settles on your behalf after you sign the order. Monvera takes whichever one can actually fill, and if the direct route fails on a given name, the leg retries through the router-settled one rather than dropping out of your plan.
...
<Aside type="note">
A router-settled fill can arrive as a wrapped token that unwraps within about 1 to 15 minutes. It is already bought, already yours, and already counted in your balance. It just cannot be sold until it lands. Nothing is stuck.
</Aside>

Each name needs about $11 per leg to clear the makers' minimum, which is why you can start from $1 but a small amount buys fewer names rather than more, smaller ones.
...
The one place the stack becomes visible is when a fill lands wrapped and takes a few minutes to unwrap, and even then it is already counted as yours.
